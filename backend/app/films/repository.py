...
class FilmRepository:
    ...
    def add_film(self, film: Film) -> None:
        ...
        work: no ORM ``relationship()`` links ``Film`` to its titles or to
        its director links (see :meth:`delete_film`), so the unit of work has
        no dependency to derive an insert order from and falls back to mapper
        name — under which ``FilmDirector`` sorts *before* ``Film`` and would
        hit the foreign key before the film row exists. A flush is not a commit:
