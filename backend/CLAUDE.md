...
**The repo is in M1 ("Core domain").** The nine §5.2 tables and their migration exist; `rewatch/` is still an empty stub (M4). Do not add domain logic to a milestone that doesn't own it (see the out-of-scope table in `docs/milestones/MILESTONE_M1_V1.md`).
...
- `core/db.py` — SQLAlchemy 2.x plumbing: lazily-cached engine/session factory (importing this module never touches the DB), the request-scoped `get_session()` generator dependency, and the typed declarative `Base`. The nine domain models register on `Base.metadata`; the guard test `test_metadata_defines_exactly_the_nine_domain_tables` keeps a stray model from silently widening the schema.
