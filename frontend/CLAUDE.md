...
- Cite design sections and requirement IDs (`§6.5`, `FR-EXT-02`) as pointers in comments — a reference, not a paraphrase of the section. Comment length follows the `code-docs` skill: a one-line summary by default, prose only for the why or the non-obvious.
